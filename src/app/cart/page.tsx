      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 animate-pulse">
          <div className="lg:col-span-8 space-y-6">
            <div className="h-8 bg-bg-subtle rounded-md w-1/4" />
            {[1, 2].map((i) => (
              <div key={i} className="h-44 bg-bg-subtle rounded-3xl" />
            ))}
          </div>
          <div className="lg:col-span-4 h-72 bg-bg-subtle rounded-3xl" />
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 space-y-10">
      ...
      {loading && !cart ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 items-start animate-pulse">
          ...
          <div className="lg:col-span-4 h-64 bg-bg-subtle rounded-2xl" />
        </div>
      ) : error ? (
        <div className="p-8 text-center text-status-danger-accent bg-status-danger-bg rounded-2xl max-w-md mx-auto">
          <p className="text-sm font-medium">{error}</p>
        </div>
      ) : isEmpty ? (
        /* 2. Empty State */
        <div className="card-soft max-w-lg mx-auto p-8 sm:p-12 bg-white border border-border-default">
          <EmptyState
            size="md"
            icon="🛒"
            title="Your Cart is Empty"
            description="Nothing here yet. Explore our mindful coloring collection and let your creativity flow."
            primaryAction={{
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 items-start">
          ...
          <div className="lg:col-span-8 space-y-5">
            ...
            {hasUnavailableItems && (
              <div className="p-4 bg-status-danger-bg border border-status-danger-accent/30 rounded-2xl flex items-start gap-3 text-xs sm:text-sm text-status-danger-accent animate-in fade-in">
            ...
            {items.map((item: CartItemDetail) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`card-soft p-5 sm:p-6 space-y-5 bg-white border border-border-default transition-opacity ${
                    isUpdating ? 'opacity-50' : 'opacity-100'
                  }`}
                >
                  <div className="flex flex-col sm:flex-row gap-5 items-start">
                    {/* Item Image */}
                    <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl bg-bg-subtle border border-border-default overflow-hidden flex-shrink-0 flex items-center justify-center">
                      {item.primaryImage ? (
                        <img
                    ...
                    <div className="flex-grow space-y-3 w-full">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <Link href={`/products/${item.slug}`} className="hover:text-action-primary transition-colors">
                            <h3 className="font-heading font-bold text-base sm:text-lg text-text-primary leading-snug">
                              {item.productName}
                            </h3>
                          </Link>
                        ...
                        <span className="font-heading font-bold text-base sm:text-lg text-action-primary">
                          {formattedLineTotal}
                        </span>
                      </div>
                      ...
                      {item.isAvailable === false && (
                        <div className="p-3 rounded-2xl bg-status-danger-bg border border-status-danger-accent/30 flex items-center justify-between text-xs text-status-danger-accent">
                          <span className="font-heading font-semibold">⚠️ Product currently unavailable (out of stock or discontinued)</span>
                          <button
                            type="button"
                            onClick={() => handleRemoveItem(item.id)}
                            className="font-heading font-bold underline hover:text-status-danger-deep ml-2 cursor-pointer"
                          >
                            Remove
                          </button>
                        </div>
                      ...
                      {item.requiresCustomization && (
                        <div
                          className={`p-3.5 rounded-2xl text-xs space-y-2 border ${
                            isCustomMissing
                              ? 'bg-status-danger-bg border-status-danger-accent/30 text-status-danger-accent'
                              : 'bg-bg-accent border-border-default text-text-primary'
                          }`}
                        >
                          <div className="flex items-center justify-between">
                            ...
                            <button
                              ...
                              className="text-[11px] font-heading font-bold text-action-primary hover:text-action-primary-hover underline"
                            >
                          ...
                          {item.customization?.notes && (
                            <p className="italic text-text-secondary">"{item.customization.notes}"</p>
                          )}
                          ...
                          {item.customization && item.customization.assets.length > 0 ? (
                            ...
                            <p className="text-[11px] font-medium text-status-danger-accent">
                      ...
                      {item.supportsThemeCustomization && (
                        item.themeCustomization && item.themeCustomization.selectedThemeIds.length > 0 ? (
                          <div className="p-3.5 rounded-2xl text-xs space-y-1 bg-bg-accent border border-border-accent/20 text-text-primary">
                            <div className="font-heading font-bold flex items-center gap-1.5 text-brand-rose">
                              <span>🎨</span>
                          ...
                          <div className="p-3.5 rounded-2xl text-xs space-y-2 bg-status-danger-bg border border-status-danger-accent/30 text-status-danger-accent">
                            <div className="flex items-center justify-between">
                              <span className="font-heading font-bold flex items-center gap-1.5">
                                <span>🎨</span>
                      ...
                      {item.addons && item.addons.length > 0 && (
                        <div className="space-y-1.5 pt-2 border-t border-border-default">
                          <span className="text-[11px] font-heading font-bold text-text-tertiary uppercase tracking-wider block">
                            Added extras:
                          </span>
                          {item.addons.map((a) => (
                            <div
                              key={a.id}
                              className="text-xs flex items-center justify-between text-text-secondary bg-bg-subtle px-3 py-2 rounded-xl"
                            >
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-border-default">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-heading font-bold text-sm w-6 text-center text-text-primary">
                            {item.quantity}
                          </span>
                          <button
                            type="button"
                        ...
                        <button
                          ...
                          disabled={isUpdating}
                          onClick={() => handleRemoveItem(item.id)}
                          className="text-xs font-heading font-semibold text-status-danger-accent hover:text-status-danger-deep transition-colors"
                        >
                          Remove Item
                        </button>
                      </div>
                ...
                hasUnavailableItems ? (
                  <div className="space-y-2 pt-2">
                    <Button
                      ...
                      size="lg"
                      disabled
                      className="w-full justify-center shadow-md font-heading font-bold rounded-full opacity-50 cursor-not-allowed"
                    >
                      Unavailable Items in Cart
                    </Button>
                    <p className="text-[11px] text-center text-status-danger-accent">
      ...
      {editingCustomizationItem && (
        <div className="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="relative w-full max-w-lg rounded-3xl bg-white p-6 sm:p-8 space-y-6 shadow-2xl border border-border-default">
              <div className="flex items-center justify-between pb-4 border-b border-border-default">
                ...
                <button
                  type="button"
                  onClick={() => setEditingCustomizationItem(null)}
                  className="w-8 h-8 rounded-full bg-bg-subtle hover:bg-bg-brand text-text-secondary flex items-center justify-center text-sm font-bold"
                >
                  ✕
                </button>
              </div>
              ...
              {/* Upload Drop area */}
              <div className="space-y-2">
                <label className="block text-xs font-heading font-bold text-text-primary">
                  Add Photos
                </label>
                <div className="border-2 border-dashed border-border-default hover:border-border-brand bg-bg-default rounded-2xl p-5 text-center transition-colors">
                  <input
                    type="file"
