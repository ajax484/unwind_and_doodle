...
export async function GET(
  ...
  try {
    ...
    const bundleComponentsByItem = new Map<string, BundleComponentRow[]>();
    for (const bc of (bundleComponents || []) as BundleComponentRow[]) {
      ...
      bundleComponentsByItem.get(bc.order_item_id)!.push(bc);
    }
    ...
    const formattedItems = (orderItems || []).map((item) => {
      ...
      return {
        ...
        bundleComponents: itemBundleComps.map((bc) => ({
